...
export default function Home() {
  ...
  const numbers = useMemo(() => {
    ...
    return {
      price, cost, logistics, extra, commission, profit,
  ...
  return (
    <div className={dark ? "dark" : ""}>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex">
        <aside className="fixed left-0 top-0 h-screen w-[300px] bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-white/10 px-5 py-6 flex flex-col justify-between overflow-y-auto">
          <div>
        ...
        <main id="home" className="ml-[300px] min-h-screen flex-1 px-8 xl:px-10 py-6 max-w-[1500px]">
          <header className="min-h-16 flex flex-wrap items-center justify-between gap-4 mb-6">
            <button onClick={() => alert("Меню уже открыто слева")} className="icon-btn">
              <Menu size={23} />
            </button>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 px-5 py-3 flex items-center gap-4 shadow-sm">
                ...
                <button onClick={() => setCredits(50)} className="text-sm font-semibold text-violet-700 dark:text-violet-300">Пополнить</button>
              </div>
          ...
          <section className="mb-8">
            <h1 className="text-4xl xl:text-[42px] leading-[1.1] font-black tracking-tight mb-3 text-slate-950 dark:text-white">
          ...
          <section className="grid grid-cols-1 2xl:grid-cols-[0.9fr_1.35fr] gap-6 mb-8">
            <div id="calculator" className="card scroll-mt-8">
            ...
            <div id="results" className="card scroll-mt-8">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
                <h2 className="card-title">Результаты расчёта</h2>
                <button onClick={saveCalculation} className="btn-soft">
              ...
              <div className="rounded-2xl border border-amber-200 dark:border-amber-400/20 bg-amber-50 dark:bg-amber-500/10 p-5 mb-4">
                <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                  <div className="text-amber-700 dark:text-amber-300 font-bold flex items-center gap-2">
                    <Star size={18} fill="currentColor" />
                    AI-рекомендация
                  </div>
                  <button onClick={() => alert(advice)} className="btn-soft-white">Подробнее</button>
                </div>
          ...
          <section id="generator" className="grid grid-cols-1 2xl:grid-cols-2 gap-6 mb-8 scroll-mt-8">
            <div className="card">
          ...
          <section id="articles" className="scroll-mt-8 pb-8">
...
function ActionCard({ icon: Icon, title, text, color, onClick }) {
  return (
    <button onClick={onClick} className="action-card">
      ...
      <ArrowRight size={21} className="text-violet-600 shrink-0" />
    </button>
  )
}
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input value={value} onChange={(e) => onChange(e.target.value)} className="input pr-9" />
...
function Metric({ title, value, badge }) {
  return (
    <div className="rounded-2xl p-5 border bg-violet-50 dark:bg-violet-500/10 border-violet-100 dark:border-violet-400/20 min-w-0">
      <div className="text-sm text-slate-600 dark:text-slate-300 mb-4 leading-5">{title}</div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="text-4xl xl:text-[42px] leading-none font-black tracking-tight text-violet-700 dark:text-violet-300 break-words">
          {value}
        </div>
...
function MiniStat({ title, value }) {
  return (
    <div className="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 p-4 min-w-0">
      <div className="text-xs leading-5 text-slate-500 dark:text-slate-400 mb-2 min-h-[20px]">{title}</div>
      <div className="font-extrabold text-lg leading-tight break-words">{value}</div>
    </div>
  )
}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        ...
        <button disabled={disabled} onClick={onCopy} className="btn-soft-white disabled:opacity-40">
...
function Article({ title, color = "violet" }) {
  ...
  return (
    <button onClick={() => alert("Статья будет добавлена позже")} className="article-card">
      ...
      <ArrowRight size={22} className="text-violet-600 shrink-0" />
    </button>
  )
}
